							...
							<div class="dropdown nav-item main-header-notification">
								<a class="new nav-link" href="#">
									...
									@if($totalUnreadNotificationsCount > 0)
										<span class="pulse">{{ $totalUnreadNotificationsCount }}</span>
									@endif
								</a>
								<div class="dropdown-menu">
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
											...
											<a href="{{ route('notifications.markAllAsRead') }}" class="badge badge-pill badge-warning mr-auto my-auto float-left">تحديد الكل كمقروء</a>
										</div>
										<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">لديك {{ $totalUnreadNotificationsCount }} إشعار غير مقروء</p>
									</div>
									<div class="main-notification-list Notification-scroll">
										@forelse($unreadNotifications as $notification)
											<a class="d-flex p-3 border-bottom" href="{{ route('notifications.markAsRead', $notification->id) }}?redirect_to={{ urlencode(route('notifications.index')) }}">
												<div class="notifyimg bg-{{ $notification->type == 'invoice' ? 'pink' : ($notification->type == 'task' ? 'purple' : ($notification->type == 'system' ? 'primary' : 'info')) }}">
													{{-- يمكنك تغيير الأيقونات بناءً على نوع الإشعار --}}
													@if($notification->type == 'invoice')
														<i class="la la-file-alt text-white"></i>
													@elseif($notification->type == 'task')
														<i class="la la-clipboard-list text-white"></i>
													@elseif($notification->type == 'payment')
														<i class="la la-money-bill-alt text-white"></i>
													@else
										...
										@empty
											<div class="d-flex p-3">
												<div class="mr-3 text-center w-100 text-muted">
